(function () {
  ...
  // Matches release.yml's "Collect build artifacts" step naming:
  // TS4RLS-<platform>[.exe] (the platform binaries) -- everything else
  // (e.g. TS4RLS_Steam_Assets.zip) falls through and is labeled with its
  // own filename instead.
  function parseAsset(name) {
    var m = /^TS4RLS-(windows|macos|linux)(?:\.exe)?$/.exec(name);
  ...
  // links, occasional bare URLs from "generate_release_notes", plus our
  // own static CLI-discontinued notice) - not full CHANGELOG.md markdown,
  // so this stays much simpler than a changelog page's own parser.
